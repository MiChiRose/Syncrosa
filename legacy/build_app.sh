...
<dict>
    ...
    <string>com.yura.igeniusai</string>
...
ICON_SRC=""
[ -f "$WORK_DIR/genius-old.icns" ] && ICON_SRC="$WORK_DIR/genius-old.icns"

if [ -n "$ICON_SRC" ]; then
    cp "$ICON_SRC" "$APP_PATH/Contents/Resources/appIcon.icns"
    cp "$ICON_SRC" "$APP_PATH/Contents/Resources/genius-old.icns"
else
    echo "Warning: genius-old.icns not found, falling back to iTunes icon..."
...
/usr/bin/touch "$APP_PATH/Contents/Resources/genius-old.icns"
