# iGeniusAI: Comparison of the Three Versions ⚖️🎵

This document outlines the architectural, aesthetic, and functional differences between the three distinct editions of **iGeniusAI**.

---

## 📊 Summary Feature Grid

| Feature | `iGeniusAI-arm` (Modern) | `iGeniusAI-legacy-objc` (Native) | `iGeniusAI-legacy` (Python) |
| :--- | :--- | :--- | :--- |
| **Tech Stack** | Swift / SwiftUI | Objective-C / AppKit (Cocoa) | Python 2.7 / Tkinter |
| **Target OS** | macOS 14.0+ (Sonoma to Sequoia) | OS X 10.9 - macOS 10.13 | OS X 10.9 - macOS 10.13 |
...
### 2. `iGeniusAI-legacy-objc` (Legacy Native Edition)
...
* **HUD Notifications:** Renders custom overlay subviews with high-contrast text and borders designed to run cleanly on OS X 10.9.

### 3. `iGeniusAI-legacy` (Legacy Python Edition)
...
* **`iGeniusAI-arm` & `iGeniusAI-legacy-objc`:** High-security profile. Credentials are saved directly into the secure macOS Keychain. The application relies on Hardened Runtime to ensure process integrity.
* **`iGeniusAI-legacy`:** Low-security profile. API keys are written in plaintext JSON format to `~/.itunes_genius_ai.json` in the user's home directory.

---

## ⚡ Performance & CPU Footprint

* **Objective-C (Legacy-ObjC):** Outstanding. Direct memory allocation, compiled native code, and background Grand Central Dispatch (GCD) queues mean zero lag on legacy Intel Core 2 Duo and early Core i3/i5 processors. Instantly launches and closes.
* **SwiftUI (ARM):** Excellent. Leverages compiler optimizations and Apple Silicon instruction sets. Extremely low power consumption.
* **Python (Legacy):** Moderate. Requires loading a Python runtime interpreter at startup, leading to slower launch times and higher memory usage.
